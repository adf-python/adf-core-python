...
from adf_core_python.core.component.module.complex import Search
...
class GatewaySearch(GatewayTargetDetector, Search):
  def __init__(
    self,
  ...
  def precompute(self, precompute_data: PrecomputeData) -> GatewaySearch:
    super().precompute(precompute_data)
    return self

  def resume(self, precompute_data: PrecomputeData) -> GatewaySearch:
    super().resume(precompute_data)
    return self

  def prepare(self) -> GatewaySearch:
    super().prepare()
    return self

  def update_info(self, message_manager: MessageManager) -> GatewaySearch:
    super().update_info(message_manager)
    return self

  def calculate(self) -> GatewaySearch:
    super().calculate()
    return self
