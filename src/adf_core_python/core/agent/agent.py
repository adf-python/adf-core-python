...
class Agent:
  ...
  def post_connect(self) -> None:
    ...
    self._ignore_time: int = int(self.config.get_value("kernel.agents.ignoreuntil", 3))
    ...
    for heard_command in heard:
      if heard_command.urn == CommandURN.AK_SPEAK:
        heard_commands.append(
          AKSpeak(
            EntityID(
              heard_command.components[ComponentControlMessageURN.AgentID].entityID
            ),
            heard_command.components[ComponentControlMessageURN.Time].intValue,
