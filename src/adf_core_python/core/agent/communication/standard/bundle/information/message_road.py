...
class MessageRoad(StandardMessage):
  ...
  def to_bits(self) -> bitarray:
    ...
    write_with_exist_flag(
      bit_array,
      self._road_blockade_repair_cost if self._road_blockade_repair_cost else None,
  ...
  def from_bits(
    ...
    road_blockade_id = read_with_exist_flag(bit_array, cls.SIZE_ROAD_BLOCKADE_ENTITY_ID)
    ...
    is_passable = True if read_with_exist_flag(bit_array, cls.SIZE_PASSABLE) else False
