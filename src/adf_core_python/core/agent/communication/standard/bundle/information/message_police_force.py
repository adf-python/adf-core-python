...
class MessagePoliceForce(StandardMessage):
  ...
  def to_bits(self) -> bitarray:
    ...
    write_with_exist_flag(
      bit_array,
      self._police_force_position.get_value() if self._police_force_position else None,
  ...
  def from_bits(
    ...
    police_force_damage = read_with_exist_flag(bit_array, cls.SIZE_POLICE_FORCE_DAMAGE)
