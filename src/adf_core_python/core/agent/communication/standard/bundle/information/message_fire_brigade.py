...
from rcrscore.entities import EntityID, FireBrigade
...
class MessageFireBrigade(StandardMessage):
  ...
  SIZE_FIRE_BRIGADE_WATER: int = 32
  ...
  def __init__(
    ...
    self._fire_brigade_entity_id: Optional[EntityID] = fire_brigade.get_entity_id()
  ...
  def to_bits(self) -> bitarray:
    ...
    write_with_exist_flag(
      bit_array,
      self._fire_brigade_position.get_value() if self._fire_brigade_position else None,
  ...
  def from_bits(
    ...
    fire_brigade_damage = read_with_exist_flag(bit_array, cls.SIZE_FIRE_BRIGADE_DAMAGE)
    ...
    fire_brigade_water = read_with_exist_flag(bit_array, cls.SIZE_FIRE_BRIGADE_WATER)
