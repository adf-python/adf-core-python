...
class AStarPathPlanning(PathPlanning):
  ...
  def get_path(
    ...
    while open_set:
      current: EntityID = min(open_set, key=lambda x: f_score.get(x, float("inf")))
      if current == to_entity_id:
        return self.reconstruct_path(came_from, current)

      open_set.remove(current)
      for neighbor in self._graph.get(current, []):
        tentative_g_score: float = g_score[current] + self.distance(current, neighbor)
        if tentative_g_score < g_score.get(neighbor, float("inf")):
          came_from[neighbor] = current
          g_score[neighbor] = tentative_g_score
          f_score[neighbor] = tentative_g_score + self.heuristic(neighbor, to_entity_id)
