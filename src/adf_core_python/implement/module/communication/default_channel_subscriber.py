...
class DefaultChannelSubscriber(ChannelSubscriber):
  ...
  def get_channel_number(
    ...
    if agent_type == EntityURN.FIRE_BRIGADE or agent_type == EntityURN.FIRE_STATION:
      agent_index = 1
    elif agent_type == EntityURN.POLICE_FORCE or agent_type == EntityURN.POLICE_OFFICE:
      agent_index = 2
    elif (
      agent_type == EntityURN.AMBULANCE_TEAM or agent_type == EntityURN.AMBULANCE_CENTER
    ):
      agent_index = 3
