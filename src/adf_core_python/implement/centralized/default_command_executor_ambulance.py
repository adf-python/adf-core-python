...
from rcrscore.entities import AmbulanceTeam, Area, Civilian, EntityID, Human, Refuge
    ...
    match self._command_type:
      ...
      case self.ACTION_LOAD:
        ...
        if position is not None:
          if position in self._world_info.get_entity_ids_of_types([AmbulanceTeam]):
            return True
        ...
        if self._target is not None:
          ...
          if isinstance(target_entity, Area):
            ...
            if human.get_hp() == 0:
              return True
            self._command_type = (
              isinstance(human, Civilian) and self.ACTION_LOAD or self.ACTION_RESCUE
            )
            return self._is_command_completed()
        return True
      case _:
