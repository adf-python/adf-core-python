...
class DefaultExtendActionClear(ExtendAction):
  ...
  def _get_rescue_action(
    ...
    for agent_entity in agent_entities:
      ...
      human_x = human.get_x()
      human_y = human.get_y()
      if human_x is None or human_y is None or police_x is None or police_y is None:
        continue
      ...
      for blockade in blockades:
        ...
        if self._is_intersecting_area(police_x, police_y, human_x, human_y, road):
          ...
          if isinstance(action, ActionClear):
            ...
            if clear_blockade is not None:
              if self._is_intersecting_blockades(blockade, clear_blockade):
            ...
            if self._is_intersecting_blockade(
              ...
              min_distance = distance
              move_action = ActionMove([road.get_entity_id()], human_x, human_y)
  ...
  def _is_inside(self, x: float, y: float, apexes: list[int]) -> bool:
    point = Point(x, y)
    polygon = Polygon([(apexes[i], apexes[i + 1]) for i in range(0, len(apexes), 2)])
  ...
  def _get_intersect_edge_action(
    ...
    for point in move_points:
      if not self._is_intersecting_area(agent_x, agent_y, point[0], point[1], road):
        if not self._is_intersecting_area(point_x, point_y, point[0], point[1], road):
    ...
    if best_point is not None:
      ...
      for blockade in self.world_info.get_blockades(road):
        if self._is_intersecting_blockade(start_x, start_y, bp_x, bp_y, blockade):
          if self._is_intersecting_blockade(
            start_x, start_y, clear_x, clear_y, blockade
          ):
            if action_clear is None:
              ...
              if clear_blockade is not None and self._is_intersecting_blockades(
                blockade, clear_blockade
              ):
                return ActionClear(clear_blockade)
              return action_clear
          elif action_move is None:
            action_move = ActionMove([road.get_entity_id()], int(bp_x), int(bp_y))
  ...
  def _is_intersecting_blockade(
    ...
    for i in range(0, len(apexes) - 3, 2):
      line1 = LineString([(apexes[i], apexes[i + 1]), (apexes[i + 2], apexes[i + 3])])
  ...
  def _get_area_clear_action(
    ...
    for blockade in blockades:
      ...
      for i in range(0, len(apexes) - 2, 2):
        distance = self._get_distance(agent_x, agent_y, apexes[i], apexes[i + 1])
    ...
    if clear_blockade is not None:
      if min_point_distance < self._clear_distance:
        vector = self._scale_clear(self._get_vector(agent_x, agent_y, clear_x, clear_y))
        clear_x = int(agent_x + vector[0])
  ...
  def _get_neighbour_position_action(
    ...
    if isinstance(position, Road):
      ...
      if road.get_blockades() != []:
        ...
        vector = self._scale_clear(self._get_vector(agent_x, agent_y, mid_x, mid_y))
        clear_x = int(agent_x + vector[0])
        clear_y = int(agent_y + vector[1])
        ...
        for blockade in self.world_info.get_blockades(road):
          if self._is_intersecting_blockade(start_x, start_y, mid_x, mid_y, blockade):
            if self._is_intersecting_blockade(
              start_x, start_y, clear_x, clear_y, blockade
            ):
              if action_clear is None:
              ...
              action_move = ActionMove([road.get_entity_id()], int(mid_x), int(mid_y))
    ...
    if isinstance(target, Road):
      ...
      if target_blockade is not None and min_point_distance < self._clear_distance:
        vector = self._scale_clear(self._get_vector(agent_x, agent_y, clear_x, clear_y))
        clear_x = int(agent_x + vector[0])
        clear_y = int(agent_y + vector[1])
        if self._equals_point(
